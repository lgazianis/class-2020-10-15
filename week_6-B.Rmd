...
joint_distribution <- tibble(urn_red = seq(0, 1000, by = 1))%>%
  mutate(paddle_red = map(urn_red, ~ rbinom(n = 100, size = paddle_size, prob = ./urn_size))) %>%
  unnest(paddle_red)

joint_distribution %>%
  ggplot(aes(x = paddle_red, y = urn_red)) + geom_point()

summary(joint_distribution)
```
...
joint_distribution %>%
  ...
  group_by(urn_red) %>%
  summarize(nrows = n(), .groups = "drop") %>%
  arrange(desc(nrows)) %>%
  mutate(prob = nrows / sum(nrows)) %>%
  ggplot(mapping = aes(x = urn_red, y = prob)) + geom_col()
```
